...
def scan_folder(path: Path, condition: Condition):
...
def report_category(category: str, files_lst: list, condition: Condition):
    with condition:
        condition.wait()
...
if __name__ == "__main__":
    ...
    try:
        condition = Condition()
        threads = []
        th1=Thread(target=scan_folder, args=(work_path, condition,))
        th1.start()
        threads.append(th1)
        ...
        for category in found_files.keys():
            if len(found_files.get(category)) > 0:
                th = Thread(target=report_category, args=(category, 
